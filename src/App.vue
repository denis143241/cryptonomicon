<template>
  <div class="container">
    <section>
      <div class="flex">
        <div class="max-w-xs">
          ...
          >
            <span
              v-for="standartTicket in seemsTickers()"
              :key="standartTicket"
              @click="addStandart(standartTicket)"
              class="
                inline-flex
                items-center
                px-2
    ...
    <section v-if="graph && tickets.includes(graph)" class="relative">
      <h3 class="text-lg leading-6 font-medium text-gray-900 my-8">
        {{ graph.name }} - USD
      </h3>
      <div class="flex items-end border-gray-600 border-b border-l h-64">
        <div
          v-for="(col, idx) in normalizeGraph()"
          :key="idx"
          :style="{ height: `${col}%` }"
...
export default {
  created() {
    const localData = localStorage.getItem("criptonomicon-list");

    if (localData) {
      this.tickets = JSON.parse(localData);
      this.tickets.forEach((el) => this.fetchTicket(el.name));
    }
  },
  async mounted() {
  ...
  methods: {
    seemsTickers() {
      ...
      for (let ticker in this.mainData) {
        ...
        if (seemsT.length >= 4) return seemsT;
      }
      return seemsT;
    },
    addStandart(t) {
    ...
    add() {
      ...
      localStorage.setItem("criptonomicon-list", JSON.stringify(this.tickets));
      this.fetchTicket(currentTicket.name);
      this.ticket = "";
    },
    fetchTicket(name) {
      setInterval(async () => {
    ...
    remove(removeTicket) {
      this.tickets = this.tickets.filter((el) => el != removeTicket);
    },
    normalizeGraph() {
      const maxValue = Math.max(...this.priceOnGraph);
      const minValue = Math.min(...this.priceOnGraph);
      return this.priceOnGraph.map(
        (price) =>
